...
CREATE OR REPLACE PROCEDURE system.LOAD_IMAGE (IMG_ID IN NUMBER, IMG_TYPE IN NUMBER, IMG_NAME IN VARCHAR2, FILE_NAME IN VARCHAR2) 
IS 
	TEMP_BLOB BLOB := EMPTY_BLOB(); 
	BFILE_LOC BFILE; 
BEGIN 
	DBMS_LOB.CREATETEMPORARY(TEMP_BLOB,TRUE,DBMS_LOB.SESSION); 
	BFILE_LOC := BFILENAME('dirTmp', FILE_NAME); 
	...
	INSERT INTO zabbix.IMAGES VALUES (IMG_ID, IMG_TYPE, IMG_NAME, TEMP_BLOB); 
	COMMIT; 
END LOAD_IMAGE;
/
...
DROP PROCEDURE system.LOAD_IMAGE;
