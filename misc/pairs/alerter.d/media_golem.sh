...
SQL="select a.userid as UserID, a.clock as Clock, t.value as Value, t.description as Name, a.message as Description, if (t.priority > 2,'critical','warning') as Severity, h.host as Host from alerts a, triggers t, functions f,items i, hosts h where a.alertid=$ZABBIX_ALERT_ID and a.triggerid=t.triggerid and f.triggerid=t.triggerid and f.itemid=i.itemid and h.hostid=i.hostid limit 1\G";
