...
descr=$(cat $OUT | grep Description | cut -f 2- -d ' ')
...
wget -q -O /dev/null "http://kohts.golem-test.yandex.net/api/event.sbml?object=$host&eventtype=$event&info=$descr$extra"
